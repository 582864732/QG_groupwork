...
namespace Geometry
{
	...
	// 创建立方体网格数据
	template<class VertexType = VertexPosNormalTex, class IndexType = DWORD>
	MeshData<VertexType, IndexType> CreateBox(int tex_mode = 0, float width = 2.0f, float height = 2.0f, float depth = 2.0f,
		const DirectX::XMFLOAT4& color = { 1.0f, 1.0f, 1.0f, 1.0f });
...
namespace Geometry
{
	...
	template<class VertexType, class IndexType>
	inline MeshData<VertexType, IndexType> CreateBox(int tex_mode,float width, float height, float depth, const DirectX::XMFLOAT4 & color)
	{
		using namespace DirectX;
		...
		else if (tex_mode == 1)
		{
			for (UINT i = 0; i < 3; i++)
			{
				vertexDataArr[i * 4].tex = XMFLOAT2(1.0 / 3.0 * i, 1.0/3.0);
				vertexDataArr[i * 4 + 1].tex = XMFLOAT2(1.0 / 3.0 * i, 0.0f);
				vertexDataArr[i * 4 + 2].tex = XMFLOAT2(1.0 / 3.0 * (i + 1), 0.0f);
				vertexDataArr[i * 4 + 3].tex = XMFLOAT2(1.0 / 3.0 * (i + 1), 1.0/3.0f);
			}
			for (UINT i = 3; i < 6; i++)
			{
				vertexDataArr[i * 4].tex = XMFLOAT2(1.0 / 3.0 * (i-3), 2.0 / 3.0);
				vertexDataArr[i * 4 + 1].tex = XMFLOAT2(1.0 / 3.0 * (i-3), 1.0/3.0);
				vertexDataArr[i * 4 + 2].tex = XMFLOAT2(1.0 / 3.0 * (i - 2), 1.0/3.0);
				vertexDataArr[i * 4 + 3].tex = XMFLOAT2(1.0 / 3.0 * (i - 2), 2.0 / 3.0f);
			}
		}
