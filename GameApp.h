...
public:
	...
	enum class ShowMode { WoodCrate, FireAnim,SixTexture };
...
private:
	...
	std::vector<std::wstring> firename;
	ComPtr<ID3D11Texture2D> textureArray;
	ComPtr<ID3D11ShaderResourceView> texs[120];
	ComPtr<ID3D11ShaderResourceView> m_pWoodCrate[3];			    // 木盒纹理
	ComPtr<ID3D11ShaderResourceView> alpha_;
